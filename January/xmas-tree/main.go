...
func XMasTree(n int) []string {

	base := n + (n - 1)
	baseStr := strings.Repeat("_", base)
	results := make([]string, n+2)
	for i := 0; i < n; i++ {
		if i == 0 {
			baseStr = baseStr[:(n-1)] + "#" + baseStr[n:]
			results[i] = baseStr
			results[len(results)-1] = baseStr
			results[len(results)-2] = baseStr
		} else {
			baseStr = baseStr[:(n-i)-1] + "#" + baseStr[n-i:]
			baseStr = baseStr[:n+(i-1)] + "#" + baseStr[n+i:]
			results[i] = baseStr
		}
	}
